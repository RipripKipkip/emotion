...
(function(window) {
	...
	function openItem(item) {
		...
		onEndTransition(zoomer, function() {
			...
			var contentItem = document.getElementById(item.getAttribute(
				'data-content'))
	...
	function applyTransforms(el, nobodyscale) {
		...
		var zoomerArea = el.querySelector('.zoomer__area'),
			...
			scaleVal = zoomerAreaSize.width / zoomerAreaSize.height < win.width / win.height ?
		...
		// apply transform
		el.style.WebkitTransform = 'translate3d(' + Number(win.width / 2 - (
				zoomerOffset.left + zoomerAreaSize.width / 2)) + 'px,' + Number(win.height /
				2 - (zoomerOffset.top + zoomerAreaSize.height / 2)) + 'px,0) scale3d(' +
			scaleVal + ',' + scaleVal + ',1)';
		el.style.transform = 'translate3d(' + Number(win.width / 2 - (zoomerOffset.left +
				zoomerAreaSize.width / 2)) + 'px,' + Number(win.height / 2 - (zoomerOffset
				.top + zoomerAreaSize.height / 2)) + 'px,0) scale3d(' + scaleVal + ',' +
			scaleVal + ',1)';
	}
	...
	function navigate(dir) {
		...
		dynamics.animate(currentEl, {
			opacity: 0,
			translateX: dir === 'right' ? -1 * currentEl.offsetWidth / 2 : currentEl.offsetWidth /
				2,
		...
		dynamics.css(nextEl, {
			opacity: 0,
			translateX: dir === 'right' ? nextEl.offsetWidth / 2 : -1 * nextEl.offsetWidth /
				2,
			rotateZ: dir === 'right' ? 10 : -10
		});
